const WATERMARK_CSS = `.watermark { position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%) rotate(-45deg); font-size: 70pt; font-weight: 900; color: rgba(160,0,0,0.07); letter-spacing: 5px; white-space: nowrap; pointer-events: none; user-select: none; }`;

function buildReprintFooter(printedBy, generatedAt, isReprint, reprintedBy, reprintedAt) {
  const parts = [];
  if (printedBy)   parts.push(`Generated by: <strong>${printedBy}</strong>`);
  if (generatedAt) {
    const label = isReprint ? 'Document Date:' : 'Date &amp; Time:';
    parts.push(`${label} ${generatedAt}`);
  }
  const reprintLine = isReprint
    ...
    : '';
  if (parts.length === 0 && !reprintLine) return '';
  return `<div class="generated-footer">${parts.join('&nbsp;&nbsp;|&nbsp;&nbsp;')}${parts.length > 0 && reprintLine ? '<br>' : ''}${reprintLine}</div>`;
}

export function printPODocument(result, { printedBy = '', generatedAt = '', isReprint = false, reprintedBy = '', reprintedAt = '' } = {}) {
...
<body>
  ...
  ${buildReprintFooter(printedBy, generatedAt, isReprint, reprintedBy, reprintedAt)}
...
export function printGDDocument(row, { printedBy = '', generatedAt = '', isReprint = false, reprintedBy = '', reprintedAt = '' } = {}) {
  if (!row) return;
...
export function printGINDocument(row, { printedBy = '', generatedAt = '', isReprint = false, reprintedBy = '', reprintedAt = '' } = {}) {
  if (!row) return;
