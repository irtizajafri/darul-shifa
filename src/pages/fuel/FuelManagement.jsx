...
export default function FuelManagement() {
  const [section, setSection] = useState(null); // null | 'vehicles' | 'generator'

  if (section === 'vehicles') {
    return <VehicleList onBack={() => setSection(null)} />;
  }
  if (section === 'generator') {
    return <GeneratorManagement onBack={() => setSection(null)} />;
  }
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-lg font-semibold text-slate-800">Generator</h2>
              <p className="text-sm text-slate-500 mt-0.5">Fuel, Oil & Daily Sheets</p>
            </div>
          </div>
