...
import { ArrowLeft, Banknote, CreditCard, Landmark, ArrowRight, Building2 } from 'lucide-react';
import { useAccountsStore } from '../../../store/useAccountsStore';
import './VoucherExpense.scss';

const METHODS = [
  { key: 'cash',  label: 'Cash Income',        Icon: Banknote   },
  { key: 'card',  label: 'Credit Card Income',  Icon: CreditCard },
  { key: 'bank',  label: 'Bank Income',         Icon: Landmark   },
];
...
export default function VoucherIncome() {
  ...
  const needsBank = mode === 'bank';

  const handleProceed = () => {
    if (!mode) return;
    if (needsBank && !bankId) return;
    navigate(`/accounts/${entityType}/transactions/voucher-income/form`, {
      state: { mode, bankId },
    });
  };
      ...
      <div className="vex__wrap">
        ...
        <div className="vex__left">
          ...
          <div className="vex__left-footer">
            ...
            <div className="vex__left-type">Income Voucher</div>
          </div>
        </div>

        {/* ── Right panel ── */}
        <div className="vex__right">
          <div className="vex__title">Select Receiving Method</div>
          <p className="vex__sub">Choose how this income will be received</p>
