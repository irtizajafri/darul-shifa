...
// Same ledger CSS as Consultant Payment History — one visual design for both.
const SPH_CSS = `
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  tr.supplier-row td { font-weight:700; padding-top:6px; }
...
function printSupplierPaymentHistory({ data, printBy }) {
  ...
  statusGroups.forEach((g) => {
    bodyHTML += `<tr class="status-row"><td colspan="5">${g.label}</td></tr>`;
    g.suppliers.forEach((s) => {
      if (isDetail) {
        bodyHTML += `
          <tr class="supplier-row">
            <td>${s.code}</td><td>${s.name}</td><td></td><td class="amt">${fmt2(s.total)}</td><td></td>
          </tr>`;
        s.rows.forEach((r) => {
          bodyHTML += `
            <tr>
        ...
        bodyHTML += `
          <tr class="supplier-row">
            <td>${s.code}</td><td>${s.name}</td><td>${s.count}</td><td class="amt">${fmt2(s.total)}</td><td></td>
          </tr>
          <tr class="sub-total-row">
            <td></td><td>${s.count}</td><td></td><td class="amt">${fmt2(s.total)}</td><td></td>
          </tr>`;
      }
    });
  });
  ...
  const cols = isDetail
    ? ['GRN #', 'Item', 'Date', 'Amount', 'VOUNO']
    : ['Sup Code', 'Sup Name', '# of GRNs', 'Amount', 'VOUNO'];

  const html = `<!DOCTYPE html><html><head><title>Supplier Payment History</title><style>${SPH_CSS}</style></head><body>
    <div class="hdr">
      <div>
...
export default function SupplierPaymentHistory() {
  ...
  const [supplierFromId, setSupplierFromId] = useState('');
  const [supplierToId, setSupplierToId] = useState('');
  ...
  const handleGenerate = async () => {
    setLoading(true);
    try {
      const from = suppliers.find((s) => String(s.id) === String(supplierFromId));
      const to = suppliers.find((s) => String(s.id) === String(supplierToId));

      const params = new URLSearchParams({ entityType, dateFrom, dateTo, reportType, paymentType });
      if (from) params.set('supplierFrom', from.code);
      if (to) params.set('supplierTo', to.code);
      if (voucherFrom) params.set('voucherFrom', voucherFrom);
      if (voucherTo) params.set('voucherTo', voucherTo);

      const r = await fetch(`${ACCOUNTS_API}/supplier-payment-history?${params}`);
      const j = await r.json();
      if (!r.ok || j?.ok === false) throw new Error(j?.message || 'Failed');
      printSupplierPaymentHistory({ data: j.data, printBy: user?.name || '' });
    } catch (err) {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.78rem', color: '#64748b', marginBottom: '1.25rem' }}>
        ...
        <span>›</span>
        <span style={{ color: '#1e293b', fontWeight: 600 }}>Supplier Payment History</span>
      </div>
      ...
      {/* Main box — Crystal Reports style, mirrors Consultant Payment History */}
      <div style={{ border: '2px solid #94a3b8', borderRadius: 6, overflow: 'hidden', maxWidth: 640, background: '#fff' }}>
        ...
        <div style={{ background: '#e2e8f0', borderBottom: '1px solid #94a3b8', padding: '0.4rem 0.75rem', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <CalendarDays size={14} color="#475569" />
          <span style={{ fontSize: '0.78rem', color: '#334155', fontWeight: 600 }}>Reports › Supplier Payments</span>
        </div>
        ...
        <div style={{ padding: '1.25rem 1.5rem 1rem' }}>
          ...
          {/* Supplier code range */}
          <div style={rowStyle}>
            <span style={fieldLabelStyle}>Supplier Code</span>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flex: 1 }}>
              <span style={{ fontSize: '0.75rem', color: '#64748b' }}>From</span>
              <div style={{ flex: 1 }}>
                <SearchableSelect
                  options={suppliers} value={supplierFromId} onChange={setSupplierFromId}
                  placeholder="(first)" getLabel={(s) => `${s.code} — ${s.name}`} getKey={(s) => s.id}
                />
              </div>
              <span style={{ fontSize: '0.75rem', color: '#64748b' }}>To</span>
              <div style={{ flex: 1 }}>
                <SearchableSelect
                  options={suppliers} value={supplierToId} onChange={setSupplierToId}
                  placeholder="(last)" getLabel={(s) => `${s.code} — ${s.name}`} getKey={(s) => s.id}
                />
              </div>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', borderTop: '1px solid #e2e8f0', paddingTop: '0.85rem', marginTop: '1rem' }}>
            ...
            >
              <CalendarDays size={13} /> {loading ? 'Loading…' : 'Supplier Payment History'}
            </button>
          </div>
        </div>
      </div>
