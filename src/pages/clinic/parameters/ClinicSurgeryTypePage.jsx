...
export default function ClinicSurgeryTypePage() {
  const { surgeryTypes, loading, fetchSurgeryTypes, createSurgeryType, updateSurgeryType, deleteSurgeryType } =
    useClinicStore();
  ...
  const filtered = surgeryTypes.filter((c) =>
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await updateSurgeryType(editing.id, { name });
        toast.success('Surgery Type updated');
      } else {
        await createSurgeryType({ name });
  ...
  async function handleDelete(item) {
    try {
      await deleteSurgeryType(item.id);
      toast.success('Surgery Type deleted');
    } catch (err) {
      toast.error(err.message);
    } finally {
  ...
  return (
    <div className="clinic-parameter-page">
      ...
      <div className="cpp-body">
        ...
        <div className="cpp-table-wrap">
          {loading ? (
            ...
            <p className="cpp-empty">No surgery types found.</p>
          ) : (
      ...
      <Modal isOpen={showModal} onClose={closeModal} title={editing ? 'Edit Surgery Type' : 'Add Surgery Type'} size="sm">
        <div className="flex flex-col gap-4">
          <Input
            ...
            placeholder="e.g. General Surgery, Cardiac Surgery"
      ...
      <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Delete Surgery Type" size="sm">
