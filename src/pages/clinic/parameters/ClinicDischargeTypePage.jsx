...
export default function ClinicDischargeTypePage() {
  const { dischargeTypes, loading, fetchDischargeTypes, createDischargeType, updateDischargeType, deleteDischargeType } =
    useClinicStore();
  ...
  useEffect(() => { fetchDischargeTypes(); }, [fetchDischargeTypes]);

  const filtered = dischargeTypes.filter((c) =>
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await updateDischargeType(editing.id, { name });
        toast.success('Discharge Type updated');
      } else {
        await createDischargeType({ name });
  ...
  async function handleDelete(item) {
    try {
      await deleteDischargeType(item.id);
    ...
    <div className="clinic-parameter-page">
      ...
      <div className="cpp-body">
        <PageHeader
          ...
          actionLabel="Add Discharge Type"
          actionIcon={Plus}
          onAction={openAdd}
        />
        ...
        <div className="cpp-toolbar">
          <div className="cpp-search">
            ...
            <input
              type="text"
              placeholder="Search discharge types..."
              value={query}
        ...
        <div className="cpp-table-wrap">
          {loading ? (
            ...
            <p className="cpp-empty">No discharge types found.</p>
          ) : (
      ...
      <Modal isOpen={showModal} onClose={closeModal} title={editing ? 'Edit Discharge Type' : 'Add Discharge Type'} size="sm">
        <div className="flex flex-col gap-4">
          <Input
            ...
            placeholder="e.g. Discharge, LAMA, Expired"
      ...
      <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Delete Discharge Type" size="sm">
