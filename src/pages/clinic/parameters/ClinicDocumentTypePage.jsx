...
export default function ClinicDocumentTypePage() {
  const { documentTypes, loading, fetchDocumentTypes, createDocumentType, updateDocumentType, deleteDocumentType } =
    useClinicStore();
  ...
  const filtered = documentTypes.filter((c) =>
  ...
  async function handleSave() {
    ...
    try {
      if (editing) {
        await updateDocumentType(editing.id, { name });
  ...
  return (
    <div className="clinic-parameter-page">
      ...
      <div className="cpp-body">
        ...
        <div className="cpp-table-wrap">
          {loading ? (
            ...
            <p className="cpp-empty">No document types found.</p>
          ) : (
      ...
      <Modal isOpen={showModal} onClose={closeModal} title={editing ? 'Edit Document Type' : 'Add Document Type'} size="sm">
        <div className="flex flex-col gap-4">
          <Input
            ...
            placeholder="e.g. CNIC Copy, Lab Report"
      ...
      <Modal isOpen={!!confirmDelete} onClose={() => setConfirmDelete(null)} title="Delete Document Type" size="sm">
