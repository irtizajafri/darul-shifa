...
// Consultant OPD is deliberately its own standalone component/print pipeline —
// NOT a variant of GeneralOPD.jsx — so it can carry the Clinical Record Form
// print and any future Consultant-only changes without touching General OPD /
// Lab / Ultrasound / Dental / etc, which all still share GeneralOPD.jsx.
...
function printClinicalRecordForm() {
...
function ClinicalRecordPrintTemplate({ visit, consultantName, barcodeDataUrl }) {
  ...
  const ageSex = [
    ...
    visit.gender ? (visit.gender === 'male' ? 'M' : 'F') : '',
  ].filter(Boolean).join(' / ');

  return (
    <div className="copd-crf">
      <div className="copd-crf-title">OUTPATIENT DEPARTMENT (OPD) - CLINICAL RECORD FORM</div>

      <div className="copd-crf-consultant">CONSULTANT NAME: <span className="copd-crf-dln">{consultantName}</span></div>

      <div className="copd-crf-row">
        <div className="copd-crf-grp"><strong>Patient Name:</strong> <span className="copd-crf-dln">{visit.patientName}</span></div>
        <div className="copd-crf-grp"><strong>Age/Sex:</strong> <span className="copd-crf-dln copd-crf-dln--sm">{ageSex}</span></div>
        <div className="copd-crf-grp"><strong>Date/TIME</strong> <span className="copd-crf-dln copd-crf-dln--md">{dateStr}&nbsp;&nbsp;{timeStr}</span></div>
      </div>

      <div className="copd-crf-row">
        <div className="copd-crf-grp"><strong>MRN / SLIP No:</strong> <span className="copd-crf-dln">{visit.serialNo}</span></div>
        <div className="copd-crf-barcode">
          {barcodeDataUrl ? <img src={barcodeDataUrl} alt="barcode" /> : 'BAR CODE'}
        </div>
        <div className="copd-crf-grp"><strong>Contact:</strong> <span className="copd-crf-dln">{visit.phoneNo}</span></div>
      </div>

      <div className="copd-crf-section-hdr">VITAL SIGNS &amp; GENERAL EXAMINATION</div>
      <table className="copd-crf-vitals">
        <tbody>
          <tr>
            <td className="vp">Blood Pressure (BP):</td><td><span className="vm" /> mmHg</td>
            <td className="vp">Pulse Rate:</td><td><span className="vm" /> bpm</td>
          </tr>
          <tr>
            <td className="vp">Temperature:</td><td><span className="vm" /> °F</td>
            <td className="vp">Respiratory Rate:</td><td><span className="vm" /> /min</td>
          </tr>
          <tr>
            <td className="vp">Weight:</td><td><span className="vm" /> kg</td>
            <td className="vp">SpO2:</td><td><span className="vm" /> %</td>
          </tr>
        </tbody>
      </table>

      <div className="copd-crf-body">
        <div className="copd-crf-body-left">
          <div className="copd-crf-section-hdr">General Physical Examination / Notes / Prescription</div>
          {blankLines(30)}
        </div>
        <div className="copd-crf-body-right">
          <div className="copd-crf-right-hdr">3. INVESTIGATIONS REQUIRED (LAB / RADIOLOGY/ULTRASOUND)</div>
          {blankLines(32)}
        </div>
      </div>

      <div className="copd-crf-footer">
        <span>NEXT VISIT DATE <span className="copd-crf-dln copd-crf-dln--md" /></span>
        <span>DR NAME <span className="copd-crf-dln copd-crf-dln--lg" /></span>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="copd-crf-print-area">
        <ClinicalRecordPrintTemplate visit={crfVisit} consultantName={crfConsultantName} barcodeDataUrl={crfBarcodeDataUrl} />
      </div>
    </>
  );
}
