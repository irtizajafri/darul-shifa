...
import { ChevronDown, ChevronUp, ClipboardList, Download, Eye, EyeOff, FileText, PackageCheck, Pencil, Plus, Printer, Search, X } from 'lucide-react';
...
export default function GoodsIssue() {
  ...
  const [gdPatientName, setGdPatientName] = useState('');
  ...
  const [ignoringGDId, setIgnoringGDId] = useState(null); // tracks which GD is being toggled
  ...
  const handleCreateGD = async (e, andPrint = false) => {
    ...
    try {
      const result = await createGDBatch({
        ...
        patientName: gdAdmissionEnabled ? gdPatientName.trim() : undefined,
    ...
    <div className="p-6">
      ...
      {showGdAdmitModal && (
        <AdmissionSearchModal
          ...
          onSelect={(row) => { setGdAdmissionNumber(row.admissionNo); setGdPatientName(row.patientName || ''); setShowGdAdmitModal(false); }}
        />
      )}
          ...
          <div className="divide-y divide-slate-100 max-h-72 overflow-y-auto">
            {reprintTab === 'GD' ? (
              (gdHeaders || [])
                ...
                .slice(0, 15)
                .map((gd) => (
                  <div key={gd.id} className={`flex items-center justify-between py-2.5 px-1 ${gd.isIgnored ? 'opacity-60' : ''}`}>
                    ...
                    <div className="flex items-center gap-1.5 shrink-0 ml-2">
                      {/* Ignore / Restore — hides or shows this GD in the GIN dropdown */}
                      <button
                        onClick={async () => {
                          setIgnoringGDId(gd.id);
                          try {
                            await ignoreGDHeader(gd.id, !gd.isIgnored);
                            toast.success(gd.isIgnored ? 'GD restore — ab GIN me show hoga' : 'GD ignore — ab GIN me nahi ayega');
                          } catch (err) {
                            toast.error(err.message || 'Failed');
                          } finally {
                            setIgnoringGDId(null);
                          }
                        }}
                        disabled={ignoringGDId === gd.id}
                        title={gd.isIgnored ? 'GIN mein wapas show karo' : 'GIN mein se chupao'}
                        className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium rounded-md border transition-colors whitespace-nowrap disabled:opacity-50 ${
                          gd.isIgnored
                            ? 'text-emerald-600 border-emerald-200 hover:bg-emerald-50'
                            : 'text-orange-600 border-orange-200 hover:bg-orange-50'
                        }`}
                      >
                        {gd.isIgnored ? <Eye className="w-3.5 h-3.5" /> : <EyeOff className="w-3.5 h-3.5" />}
                        {ignoringGDId === gd.id ? '...' : gd.isIgnored ? 'Restore' : 'Ignore'}
                      </button>
                      <button
        ...
        <Card className="mb-4" title="Create Goods Demand (GD)">
          <form ref={gdFormRef} onSubmit={handleCreateGD} className="space-y-4">
                ...
                <label className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer select-none">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      else { setGdAdmissionNumber(''); setGdPatientName(''); }
                    }}
                ...
                {gdAdmissionEnabled && (
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      value={gdPatientName}
                      onChange={(e) => setGdPatientName(e.target.value)}
                      className="px-3 py-2 border border-blue-300 rounded-md text-sm w-full focus:outline-none focus:border-blue-500"
                    />
                  </div>
                )}
            ...
            <div className="flex gap-2">
              ...
              <Button type="button" variant="secondary" label="Cancel" onClick={() => { setShowGDForm(false); setGdDepartmentId(''); setGdSelectedItems([]); setGdItemSearch(''); setGdAdmissionEnabled(false); setGdAdmissionNumber(''); setGdPatientName(''); setGdCommentEnabled(false); setGdComment(''); }} />
            </div>
          </form>
        </Card>
      )}
      ...
      {canGIN && showGINForm && (
        <Card className="mb-4" title="Create Goods Issuance (GIN)">
          <form ref={ginFormRef} onSubmit={handleCreateGIN} className="space-y-4">
            ...
            {selectedGDHeader?.admissionNumber && (
              <div className="flex flex-wrap items-center gap-2">
                <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-blue-50 border border-blue-200 text-xs font-medium text-blue-700 w-fit">
                  Admission #: <span className="font-mono">{selectedGDHeader.admissionNumber}</span>
                </div>
                {selectedGDHeader.patientName && (
                  <div className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-blue-50 border border-blue-200 text-xs font-medium text-blue-700 w-fit">
                    Patient: <span>{selectedGDHeader.patientName}</span>
                  </div>
                )}
              </div>
            )}
