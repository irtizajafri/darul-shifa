// Using app's established Prisma connection
...
async function debugItemCodes() {
  try {
    console.log('\n🔍 Checking InventoryItem codes in database...\n');
    ...
    if (items.length === 0) {
      console.log('✅ Database is empty - no items exist yet\n');
      process.exit(0);
    }
    ...
    Object.entries(grouped).forEach(([key, itemsInGroup]) => {
      console.log(`\n📦 Category-Subcategory ${key} (${itemsInGroup.length} items):`);
